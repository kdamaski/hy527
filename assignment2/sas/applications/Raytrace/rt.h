...
#define MAX_PROCS	64		/* Max # of processors. 	     */
...
typedef VEC4	POINT4;			/* An x, y, z, w point. 	     */
...
typedef struct	sphere
	{
	POINT4	center; 		/* Center of sphere.		     */
	REAL	rad;			/* Radius of sphere.		     */
	REAL	rad_2;			/* Radius squared of sphere.	     */
	}
	SPHERE;
...
typedef struct	view
	{
	POINT4	eye;			/* Eye position.		     */
	POINT4	coi;			/* Center position		     */
...
typedef struct	wpjob
	...
	struct	wpjob	*next;
/* SHZ 7/1/96 */
	INT	pad1,pad2;    /* SHZ 7/1/96, for size == 32 */
	}
	WPJOB;
...
typedef struct	gmem
	{
	WPJOB	*workpool1[MAX_PROCS]; /* Ptr to heads of 
					   shared work pools.  Padded to 
					   avoid false-sharing */
	    WPJOB   *workpool2[MAX_PROCS];
...
extern	void InitRayTreeStack(INT TreeDepth, INT pid);
...
extern	long GetOneJob(struct rayjob *job,INT pid, INT who);
extern	long GetJobs(struct rayjob *job,INT pid);

/* Following is added by SHZ6/26/96 */

int     task_steal[64][128];
int     task_steal_num[64];
