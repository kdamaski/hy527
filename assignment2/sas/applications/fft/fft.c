...
int test_result = 0; /* rudro - this was 0 but I wan to test the result ! */
int doprint = 0;     /* rudro - this was 0 but I wan to see the result ! */
int dostats = 0;     /* rudro - this was 0 but I want stats ! */
int transtime = 0;
int transtime2 = 0;
int avgtranstime = 0;
int avgcomptime = 0;
...
void allocate(double *, double *, double *, int, int, int);
...
main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "i:p:m:n:l:stoh")) != -1) {
    ...
    case 'i':
      T = atoi(optarg);
      if (T < 1) {
        printerr("Number of iterations must be >= 1\n");
        exit(-1);
      }
      break;
    case 'n':
  ...
  for (i = 0; i < P; i++) {
    int first = rootN * i / P;
    int last = rootN * (i + 1) / P;
    allocate(x, trans, umain2, i, first, last);
  }
#endif
...
#if 0
  /* angelos; @@@ this will make it work for the case n=1,p=1 as well*/
...
void SlaveStart() {
  ...
  upriv = (double *)malloc(2 * (rootN - 1) * sizeof(double) + PAGE_SIZE);
  if (upriv == NULL) {
...
#if 0
  /* @@@ angelos: perform one extra fft to warmup */
      ...
      if ((MyNum == 0) || (dostats)) {
        {
          unsigned long now;
          gettimeofday(&the_time, NULL);
          now = ((the_time.tv_sec - 879191283) * 1000) +
                (the_time.tv_usec / 1000);
          initdone = (unsigned int)now;
        };
      }
      {
        unsigned long now;
        gettimeofday(&the_time, NULL);
        now =
            ((the_time.tv_sec - 879191283) * 1000) + (the_time.tv_usec / 1000);
...
void allocate(x, scratch, u, num, first, last) double *x;
double *scratch;
double *u;
int num;
int first;
