...
int i_times = 1;        /* @@@dongming: number of loops to perform lu */
...
void printerr(char *);
void round_robin(u_int start, u_int size, u_int * p, u_int pnum);
...
pthread_mutex_unlock(&barrier_mutex);
  ...
  barrier_init(&Global->start);;
  pthread_mutex_init((pthread_mutex_t*)&(Global->idlock), NULL);;
  InitA(rhs);			/* rudro : Moved this here ... was after 
pthread_create(&threads[thread_count],NULL,,NULL);
thread_count++;
 */
  Global->id = 0;
...
/*  InitA(rhs);  rudro : Moved this to before 
pthread_create(&threads[thread_count],NULL,,NULL);
thread_count++;
 to avoid allocation problems */
  if (doprint) {
...
pthread_mutex_lock((pthread_mutex_t*)&(Global->idlock));

  {
	
pthread_mutex_lock((pthread_mutex_t*)&(id_lock));
;
	MyNum = id++;
	
pthread_mutex_unlock((pthread_mutex_t*)&(id_lock));
;
};
//    MyNum = Global->id;
  Global->id++;
  
pthread_mutex_unlock((pthread_mutex_t*)&(Global->idlock));
...
void
bdiv(a, diag, stride_a, stride_diag, dimi, dimk)
     double *a;
     double *diag;
     int stride_a;
     int stride_diag;
...
void
bmodd(a, c, dimi, dimj, stride_a, stride_c)
     double *a;
     double *c;
     int dimi;
     int dimj;
...
void
bmod(a, b, c, dimi, dimj, dimk, stridea, strideb, stridec)
     double *a;
     double *b;
     double *c;
     int dimi;
...

{
  ...
  int jbs, skip;
  			...
  			/* bilas: avoid making thread 0 the owner of all pages */
    srand48((long)1);
  edge = n % block_size;
  for (j = 0; j < n; j++) {
    for (i = 0; i < n; i++) {
...
#ifdef RR
void
round_robin(u_int start, u_int size, u_int * p, u_int pnum)
{
  u_int addr;
